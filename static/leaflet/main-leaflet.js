...
document.getElementById("has-feedback-locations-btn").addEventListener("click", function() {
    loadLocations("/feedback/all-project-feedback/");
});
...
filterForm.addEventListener('submit', function(e) {
    ...
    // گرفتن مقادیر فرم
    let company_name_fb = document.getElementById('company').value;
    let location_fb = document.getElementById('location').value;
    let start_fb = document.getElementById('start-date').value;
    let end_fb = document.getElementById('end-date').value;

    // ساخت query string
    // let query = new URLSearchParams();
    // if (company_name_fb) query.append('company_name', company_name);
    // if (location) query.append('location_name', location);
    // if (start) query.append('start', start);
    // if (end) query.append('end', end);
    // console.log(start)
